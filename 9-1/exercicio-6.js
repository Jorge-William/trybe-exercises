...
// ao fato de o robô já estar muito ocupado com outras operações. Logo,
...
const sendMarsTemperature = () => {
  ...
  const messageSuccessfullySend = Math.random() <= 0.6;
  setTimeout(() => {
    if (messageSuccessfullySend) {
      onSuccess()
    } else {
      onError('Robot is in use')
    }
  }, messageDelay)
}
